 ...
 * @Description:
 * @Author: L LC @amov
 * @Date: 2023-09-14 10:23:19
 * @LastEditors: L LC @amov
 * @LastEditTime: 2023-09-20 15:22:48
 * @FilePath: \SFP210-gimbal-powerControl\common\iap\src\iap_check.h
 */
#ifndef __IAP_CHECK_H
#define __IAP_CHECK_H

#include "stdint.h"

static const uint32_t iapCrc32Table[256] = {
 ...
 */
static inline uint32_t iapCheckCRC32(uint8_t *pData, uint32_t Length)
{
    uint32_t nReg;
    uint32_t nTemp = 0;
    uint16_t i, n;
    ...
    for (n = 0; n < Length; n++)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            nTemp = iapCrc32Table[(uint8_t)((nReg >> 24) & 0xff)];
            nReg <<= 8;
            nReg ^= nTemp;
        }
    }
